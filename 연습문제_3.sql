...
SELECT p.player_name, p.position,
    CASE
        WHEN p.position = 'MF' THEN '미드필더'
        WHEN p.position = 'DF' THEN '수비수'
        WHEN p.position = 'FW' THEN '공격수'
        WHEN p.position = 'GK' THEN '골키퍼'
    END AS 포지션분류
FROM tbl_player p
JOIN tbl_team t
ON p.team_id = t.team_id
WHERE t.team_id IN ('K02', 'K05');
...
/*
    ...
    480개의 행이 조회됩니다.
*/
SELECT player_name, IFNULL(nation, '대한민국')
FROM tbl_player;
...
/*
    ...
    12개의 행이 조회됩니다.
*/
SELECT DISTINCT IFNULL( nation, '대한민국'), COUNT(*) 선수
FROM tbl_player
GROUP BY nation;
...
/*
    ...
    15개의 행이 조회됩니다.
*/
SELECT team_id, CONCAT(zip_code1,'-', zip_code2) 우편번호, CONCAT('(', ddd, ')', tel) tel
FROM tbl_team;
